...
class BudgetCertificateResult:
    ...
    validation_removed: float = 0.0
    feasibility_slack: float = 0.0
...
def tv_energy_float64(u: np.ndarray, eps: float = 1e-8, vectorial: bool = True) -> float:
    """Evaluate the reported TV functional without the float32 cast in ``_as_3d``."""
    u64 = np.asarray(u, dtype=np.float64)
    if u64.ndim == 2:
        u64 = u64[..., None]
    elif u64.ndim != 3:
        raise ValueError(f"Expected 2D or 3D image, got shape {u64.shape}")
    gx, gy = forward_grad(u64)
    if vectorial:
        mag = np.sqrt(np.sum(gx * gx + gy * gy, axis=2) + float(eps) ** 2)
    else:
        mag = np.sqrt(gx * gx + gy * gy + float(eps) ** 2)
    return float(np.mean(mag, dtype=np.float64))
...
def certify_candidate(
    ...
    """Return the largest numerically feasible point on the input--proposal segment.

    Feasibility decisions are independently evaluated in float64 on the exact array
    representation that is returned. ``budget_override`` exposes the user-facing
    budget directly while preserving the earlier schedule as the default.
    """
    f, squeeze = _as_3d(np.clip(image, 0.0, 1.0))
    v, _ = _as_3d(np.clip(candidate, 0.0, 1.0))
    if v.shape != f.shape:
        raise ValueError(f"candidate shape {v.shape} does not match image shape {f.shape}")
    if p is None and budget_override is None:
        if sigma_est is None:
            raise ValueError("Provide either p or sigma_est.")
        p = p_from_sigma(sigma_est, tau=tau_calib, rho=rho_calib)

    if p is None:
        p = p_from_sigma(sigma_est, tau=tau_calib, rho=rho_calib) if sigma_est is not None else 1.0
    budget = float(budget_override) if budget_override is not None else delta_q(p, alpha=alpha, gamma=gamma)
    if not np.isfinite(budget) or budget < 0.0:
        raise ValueError(f"budget must be finite and nonnegative, got {budget}")
    tol = max(float(feasibility_tolerance), 0.0)
    tv0 = tv_energy_float64(f, vectorial=vectorial)
    candidate_removed = max(0.0, tv0 - tv_energy_float64(v, vectorial=vectorial))
    if candidate_removed <= budget + tol:
        out = v[..., 0] if squeeze else v
        return BudgetCertificateResult(
            out, 1.0, budget, candidate_removed, float(p), True,
            validation_removed=candidate_removed,
            feasibility_slack=budget - candidate_removed,
        )
    ...
    for _ in range(int(bisection_steps)):
        eta = 0.5 * (lo + hi)
        trial = f + eta * (v - f)
        trial_removed = max(0.0, tv0 - tv_energy_float64(trial, vectorial=vectorial))
        if trial_removed <= budget + tol:
            best = trial.copy()
    ...
    validation_removed = max(0.0, tv0 - tv_energy_float64(out, vectorial=vectorial))
    if validation_removed > budget + tol:
        raise RuntimeError("Internal error: returned proposal failed the float64 budget audit")
    return BudgetCertificateResult(
        out, float(lo), budget, best_removed, float(p), False,
        validation_removed=validation_removed,
        feasibility_slack=budget - validation_removed,
    )
